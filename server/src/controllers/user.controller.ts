...
export class UserController {
  ...
  static async create(req: Request, res: Response) {
    ...
    res.status(400).json({
      error: 'Criação de utilizadores via API não é suportada. Use o registo em /sign-up.',
    });
  }
  ...
  static async getStats(req: Request, res: Response) {
    try {
      ...
      const userStats = users.map((user) => {
        const uid = user.id;
        const userSubmissions = allSubmissions.filter((s) => s.submittedBy === uid);
        ...
        const recentSubmissions = userSubmissions.filter((s) => new Date(s.submittedAt) >= thirtyDaysAgo);
        const activeDays = new Set(recentSubmissions.map((s) => new Date(s.submittedAt).toISOString().split('T')[0])).size;
