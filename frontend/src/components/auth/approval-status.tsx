...
export function ApprovalStatus() {
  ...
  const { updateUserStatus, approvalStatus: authApprovalStatus } = useAuth();
  const [data, setData] = useState<ApprovalData | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');

  // Se o auth já sabe que está aprovado (ex.: após F5), redirecionar de imediato e não mostrar "Conta aprovada"
  useEffect(() => {
    if (authApprovalStatus === 'approved') {
      navigate('/', { replace: true });
    }
  }, [authApprovalStatus, navigate]);

  const fetchStatus = useCallback(async (retryCount = 0): Promise<void> => {
    try {
  ...
  }, [data?.approvalStatus, fetchStatus]);

  useEffect(() => {
    const status = (data?.approvalStatus ?? '').toLowerCase();
    if (status === 'approved') {
      // Sync auth state (shared via AuthProvider) so ProtectedRoute won't redirect back
      updateUserStatus({ status: 'approved', emailVerified: true });
      const t = setTimeout(() => {
        navigate('/', { replace: true });
      }, 1000);
      return () => clearTimeout(t);
    }
  }, [data?.approvalStatus, updateUserStatus, navigate]);

  const status = (data?.approvalStatus ?? '').toLowerCase();

  // Já aprovado no auth (ex.: após F5) — redirecionar sem mostrar o card "Conta aprovada"
  if (authApprovalStatus === 'approved') {
    return (
          ...
          <Card className="border-0 shadow-lg shadow-black/5 bg-card">
            ...
            <CardFooter className="flex flex-col gap-2 pt-2 pb-10">
              <Button asChild variant="default" size="lg" className="w-full">
                <Link to="/sign-in">Voltar ao início de sessão</Link>
              </Button>
            </CardFooter>
          </Card>
        )}
