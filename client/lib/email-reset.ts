 ...
 *
 * Em produção: configure RESEND_API_KEY e EMAIL_FROM (ex: "noreply@seudominio.com").
 * Em desenvolvimento: o link é impresso no console (nunca em produção).
 */
        ...
        if (isDev) {
          ...
          console.log(
            ...
            isTestingRestriction
              ? '\n  → Resend em modo teste só envia para o email da conta. Para outros destinatários, verifique um domínio em resend.com/domains e defina EMAIL_FROM.'
              : '',
          );
        }
      } else if (isDev && data?.id) {
        console.log('[email-verification] Enviado com sucesso (id):', data.id);
      }
    } catch (e) {
...
<body style="font-family: system-ui, -apple-system, sans-serif; line-height: 1.6; color: #333; max-width: 480px; margin: 0 auto; padding: 24px;">
  ...
  <p style="color: #666; font-size: 14px;">Este link expira em 24 horas. Se não solicitou esta verificação, ignore este email.</p>
  <p style="color: #999; font-size: 12px; margin-top: 32px;">Se o botão não funcionar, copie e cole no navegador: ${url}</p>
</body>
</html>
`.trim();
