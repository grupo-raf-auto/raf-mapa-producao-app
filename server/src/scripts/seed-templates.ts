...
// Carregar .env quando o script é executado diretamente (npx tsx src/scripts/seed-templates.ts)
 ...
 * Procura primeiro por título; se modelType for dado, também procura por modelType
 * para garantir que o template default do modelo (ex.: crédito) receba as questões
 * mesmo que o título na BD tenha pequenas variações.
 */
async function upsertTemplate(
  ...
  let existing = await prisma.template.findFirst({ where: { title } });
  if (!existing && modelType) {
    existing = await prisma.template.findFirst({
      where: { modelType, isDefault: true },
    });
    if (existing) {
      // Alinhar título/descrição ao esperado
  ...
  if (!existing) {
    ...
    console.log(
      `  ✓ Template "${title}" criado (${modelType || 'todos os modelos'})`,
    );
  } else {
    ...
    console.log(
      `  ✓ Template "${title}" atualizado (${modelType || 'todos os modelos'})`,
    );
  }
}
