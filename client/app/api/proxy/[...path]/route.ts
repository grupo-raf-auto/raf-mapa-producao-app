...
async function handleRequest(
  ...
  try {
    ...
    let data: unknown;
    try {
      data = isJson ? await response.json() : { error: await response.text() || `HTTP ${response.status}: ${response.statusText}` };
    } catch {
    ...
    if (!response.ok && response.status === 429) {
      ...
      if (retryAfter) {
        ...
        (data as Record<string, unknown>).error = `Muitas requisições. Tente novamente em ${retryAfter} segundos.`;
      }
    }
