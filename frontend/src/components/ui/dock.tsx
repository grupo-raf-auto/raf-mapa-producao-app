...
function DockItem({ children, className }: DockItemProps) {
  ...
  const { distance, magnification, mouseX, mouseY, spring, orientation } =
    useDock();
    ...
    >
      {Children.map(children, (child) =>
        cloneElement(
          ...
          { width: size, height: size, isHovered } as any,
        ),
      )}
    </motion.div>
  );
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          animate={{
            opacity: 1,
            [isVertical ? "x" : "y"]: isVertical ? -10 : -10,
          }}
          ...
          className={cn(
            ...
            isVertical ? "-left-20 top-1/2" : "-top-6 left-1/2",
            className,
          )}
          role="tooltip"
