    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <div className="relative h-8 bg-muted rounded-lg overflow-hidden">
          <div
            className="absolute left-0 top-0 h-full bg-amber-700/60"
...
export function ReportsCompletionRadial({
  data,
}: {
  data: { month: string; count: number; totalValue: number }[];
}) {
...
export function ReportsCompletionBar({
  data,
}: {
  data: { month: string; count: number; totalValue: number }[];
}) {
  ...
  const percentage = Math.round(
    (totalCompleted / (totalCompleted + totalMissing)) * 100,
  );
...
export function SalesTimelineChart({
  ...
  return (
    <div className="space-y-4">
      {/* KPI Principal - Valor Total em Destaque */}
      <div>
